;(function(){
  ...
  var MAIN = function(){
    var btn_active = document.querySelectorAll(".links li");
    if(btn_active && btn_active.length){
      for(var i=0; i<btn_active.length; i++){
        if(btn_active[i].getAttribute("data-active") === "1"){continue;}
  ...
  MAIN.prototype.active = function(e){
    ...
    var url = urlinfo.url +"?p=" +urlinfo.query.p +"&c="+ urlinfo.query.c +"&type="+ type;
    location.href = url;
  };
