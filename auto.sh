...
mode_auto(){
  ...
  if [ -e $filepath ];then
    ...
    if [ "$CID_0" = "$CID_1" ]; then
      ...
      printf "%s\033[36m%s\033[m\n" '(auto) ' "$filepath : latest."
    
    # 差分がある場合
    elif [ "${pullonly}" = "1" ];then
      printf "%s\033[31m%s\033[m%s\n" '(auto) ' 'pull-only <- ' " : $filepath"
...
mode_push(){
  ...
  if [ -e $filepath ];then
    ...
    if [ "$CID0" != "$CID1" ]; then
      ...
      res=`git -C $filepath push --set-upstream origin $branch`
      echo "(push) $res"
...
proc_json_git(){
  ...
  jq --compact-output -r '.[]' $filename | while read LINE;do
    ...
    BRANCH=`echo $LINE | jq -r 'if .branch then .name else "master" end'`
    PULLONLY=`echo $LINE | jq -r 'if .pullonly then "1" else "0" end'`
    ...
    if [ "$FLG" = "1" ];then
      ...
      mode_auto "$DIR/$FILE" $GIT $date $BRANCH $PULLONLY
...
if [ "$type" = "" ] || [ "$type" = "system" ]; then
  ...
  if [ $ext = "json" ]; then
    # echo "json"
    proc_json_system $mode $filename $date

  elif [ $ext = "csv" ]; then
    # echo "csv"
    proc_csv_system $mode $filename $date
  fi
fi
