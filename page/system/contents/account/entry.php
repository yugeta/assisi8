...
class entry{
  ...
  public static function post(){
    ...
    $datas = array(
      // "account_id" => \mynt::exec('\lib\auth\uuid\UUID','v4',array()),
      "hash"  => $hash,
      "mode"  => "entry",
      "mail"  => $mail,
      "entry" => $update
    ...
    // // idにuuidを付与
    // $datas["id"] = \mynt::exec('\lib\auth\uuid\UUID','v4');
    ...
    // $res = \mynt::exec('\lib\mail\send',"data",array($mail,$from,$subject,$message));
  ...
  // 新規アカウント登録（メールのみ登録パターン）
  public static function post_mb(){
    $mail = $_POST["mail"];
    ...
    if($setting_res["status"] === "ok" && isset($setting_res["data"]["mail"]) && $setting_res["data"]["mail"]){
      ...
      die("Error ! Fail Mail-server-setting. (system/account/entry/post_mb)");
    }
    ...
    // メール送信
    $res = \mynt::exec('\lib\mail\common',"sendMail_mb",array($mail,$from,$subject,$message));
...
// echo "code : ".$code.PHP_EOL;
    ...
    // $wheres = array("hash"  => $code);
// var_dump($where);
    $sort   = array("entry" => "SORT_ASC");
    $resLoad = \mynt::data_load($GLOBALS["page"]["page"] , "lib_account_entry_cache" , array() , $wheres , $sort);
// echo file_get_contents("data/mynt_api/lib_account_entry_cache/be9499337b571fd4bb659a3845d3e9e6.json").PHP_EOL;
// print_r($wheres);
// print_r($resLoad);
// exit();
    ...
    //   $_REQUEST["c"] = $_POST["next"];
    //   echo \mynt::exec('\lib\html\page' , 'view' , array($_GET["f"],$_GET["p"]));
    //   exit();
    // }
    // else 
    // if(isset($_POST["redirect"]) && $_POST["redirect"]){
  ...
  public static function post_pass(){
    ...
    //   $redirect = \mynt::exec('\lib\url\common',"getUri");
    ...
    // アカウント追加処理
    $update = date("YmdHis");
// die("uuid : ".\mynt::exec('\lib\auth\uuid\UUID','v4'));
    // password_reset
